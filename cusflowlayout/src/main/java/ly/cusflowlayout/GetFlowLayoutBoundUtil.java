...
public class GetFlowLayoutBoundUtil {
	 ...
	 */
	public void setBound(int parentWidth,int pos,int count){
		...
		if (cl+childWidth+lastChildWidth+lp.rightMargin+lp.leftMargin> parentWidth) {
			if (pos==count){
				allHeight+=(childHeight+lp.topMargin+lp.bottomMargin);
				maxWidth+=(childWidth+lp.leftMargin+lp.rightMargin);
			}else{
				allHeight+=(childHeight+lp.topMargin);
				maxWidth+=(childWidth+lp.leftMargin);
			}
				row++;
				cl = 0;
				cl = cl+lp.leftMargin; 
